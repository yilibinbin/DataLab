...
中央工作区会把当前数据编辑区与公式预览、参数表和常数表放在一起；只有当前模式确实
提供这些输入时才显示对应面板。公式预览只是显示用途，实际计算仍使用原公式编辑器中的
源码表达式。
...
公式卡片中的“预览语法”只控制公式预览如何解释当前文本，可在 DataLab 兼容、Python 风格和
Mathematica 风格之间切换。这个选择不会改写公式输入框，也不会改变计算配置、拟合模型或工作区
计算哈希；真正计算仍使用输入框中的原始表达式和当前模块的安全表达式引擎。预览语法会作为工作区
UI 偏好保存，因此切换后可能出现未保存提示，但重新打开工作区时能恢复同一个预览设置。

放大预览窗口中的“高保真 LaTeX”是可选的显示路径，适合查看 `cases`、矩阵等普通数学预览不易显示
的表达式。它只使用本机已安装或已缓存的 TeX 引擎和宏包，不会为预览自动联网安装宏包；不可用时会
保留已有数学预览并显示错误提示，必要时显示源文本提示。

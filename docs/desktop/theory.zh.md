...
求根模块把方程统一表示为残差 `F(...)=0`。标量和方程组求解会根据精度设置选择可用后端；16 位及以下优先尝试 SciPy 快速路径并验证残差，失败时回退到多精度路径。

不确定度传播复用误差传递的输入解析与导数思想：

- Taylor：通过隐函数导数估计根的不确定度
- Monte Carlo：对带不确定度输入抽样，逐样本重新求根并统计根分布

输出保留机器可读的 `value`/`uncertainty` 字段，同时在人类可读结果和 LaTeX 中显示紧凑的 `值(不确定度)` 格式。

...
from shared.plotting import (  # noqa: F401
    FittingPlotLabels,
...
def render_fitting_overview(
    ...
    spec = fitting_overview_spec_from_result(
        x_plot,
        y_plot,
        fitted_series,
        residual_series,
        labels=labels or FittingPlotLabels(),
    ...
    try:
        rendered = render_fitting_overview_from_spec(
            spec,
            log_scale=log_scale,
            dpi=dpi,
            export_pdf_path=export_pdf_path,
            export_eps_path=export_eps_path,
        )
        return rendered or b""
    except OSError as exc:
        _logger.warning("render_fitting_overview: export failed: %s", exc)
        return render_fitting_overview_from_spec(spec, log_scale=log_scale, dpi=dpi) or b""

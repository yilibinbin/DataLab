...
def test_formula_preview_dialog_exposes_single_rendered_preview(qtbot):
    from app_desktop.formula_preview import FormulaPreviewDialog

    dialog = FormulaPreviewDialog(expression="a*x+b", lhs="y")
    qtbot.addWidget(dialog)

    assert not hasattr(dialog, "render_tier_combo")
    assert not hasattr(dialog, "latex_source_edit")
    assert not hasattr(dialog, "high_fidelity_render_button")
    assert dialog.formula_surface.pixmap() is not None
    assert dialog.expression_text.toPlainText() == "a*x+b"


def test_legacy_formula_tex_render_worker_module_removed() -> None:
    path = Path(__file__).resolve().parents[1] / "app_desktop" / "formula_tex_render_worker.py"
...
def test_inline_formula_preview_never_starts_tex_worker(qtbot):
    import app_desktop.formula_preview as formula_preview

    label = formula_preview.FormulaPreviewLabel()
    qtbot.addWidget(label)

    assert not hasattr(formula_preview, "FormulaTexRenderWorker")

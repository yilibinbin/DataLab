...
def test_window_latex_pdf_file_dialog_titles_use_tr():
    text = _read("app_desktop/window_latex_pdf_mixin.py")

    assert _LITERAL_ZH_FILEDIALOG_TITLE_RE.search(text) is None
    assert 'self._tr("选择 LaTeX 文件", "Select LaTeX File")' in text
    assert 'self._tr("保存 LaTeX 文件", "Save LaTeX File")' in text
    assert 'self._tr(f"选择 {engine} 可执行文件", f"Select {engine} Executable")' in text
    assert 'self._tr("缺少 pdftoppm/gs，无法生成预览", "Missing pdftoppm/gs; cannot generate preview")' in text
    assert 'self._tr("PDF 预览转换超时。", "PDF preview conversion timed out.")' in text
    assert 'QLabel(self._tr(f"页 {idx}", f"Page {idx}"))' in text
    assert 'self._tr(f"PDF 预览工具缺失: {exc}", f"PDF preview tool missing: {exc}")' in text
    assert 'self._tr(f"PDF 预览生成异常: {exc}", f"PDF preview generation error: {exc}")' in text

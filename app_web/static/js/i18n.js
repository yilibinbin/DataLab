...
(function() {
  ...
  const translations = {
    zh: {
      ...
      'fit.mcmcRefineCheckbox': 'MCMC 精修（多项式/反幂）',
      'fit.modeLabel': '拟合模式',
      ...
      'fit.modeComparison': '选定拟合比较',
      'fit.modeSelfConsistent': '自洽隐式模型 / Self-consistent',
      ...
      'fit.implicitEquationLabel': '隐式方程',
      'fit.implicitEquationPlaceholder': '如 y - A*exp(-B/x*y)',
      'fit.implicitVariableLabel': '隐式变量',
      'fit.implicitVariablePlaceholder': '如 y',
      'fit.implicitOutputLabel': '输出表达式',
      'fit.implicitOutputPlaceholder': '如 y',
      'fit.implicitParamsLabel': '参数配置 (JSON，可选)',
      'fit.implicitInitialLabel': '初值',
      'fit.implicitToleranceLabel': '收敛容差',
      'fit.implicitMaxIterLabel': '最大迭代次数',
      ...
      'fit.comparisonCandidatesLabel': '选定拟合列表 (JSON)',
      'fit.comparisonCandidatesHint': '只比较这里显式列出的拟合项；顺序按 JSON 列表保留。',
      'fit.comparisonResultsLabel': '选定拟合比较',
      'fit.comparisonResultsHint': '按输入顺序显示每个拟合项的状态和指标。',
      'fit.metricChiSquarePValue': 'χ² p 值',
      'fit.metricMaxStandardizedResidual': '最大标准化残差',
      'fit.correlationsLabel': '参数相关性',
      'fit.correlationsHint': '由协方差导出的相关系数。',
      'fit.correlationsParamI': '参数 i',
      'fit.correlationsParamJ': '参数 j',
      'fit.correlationsValue': '相关系数',
      'fit.residualsLabel': '标准化残差',
      'fit.residualsHint': '按 σ、权重或 RMSE 归一化的残差诊断。',
      'fit.residualsRow': '行',
      'fit.residualsValue': '数值',
      'fit.residualsType': '类型',
      'fit.compareOrder': '序号',
      'fit.compareId': 'ID',
      'fit.compareModel': '模型',
      'fit.compareStatus': '状态',
      'fit.compareParams': '参数数',
      'fit.compareWarnings': '提示',
      'fit.compareError': '错误',
      'fit.exportComparison': '导出比较结果 CSV',
      ...
      'stats.modeMean': '算术均值',
      'stats.modeDescriptive': '描述统计',
      'stats.modeWeighted': 'σ 加权均值',
      'stats.metricVariance': '方差',
      'stats.metricMedian': '中位数',
      'stats.metricQ1': '第一四分位数 Q1',
      'stats.metricQ3': '第三四分位数 Q3',
      'stats.metricIqr': '四分位距 IQR',
      'stats.metricMad': '中位数绝对偏差 MAD',
      'stats.metricSkewness': '偏度',
      'stats.metricExcessKurtosis': '超额峰度',
      'stats.useSample': '使用样本统计 (n-1 校正)',
      'stats.sampleHint': '不勾选则使用总体统计 (n 除数)。样本统计用于估计总体参数，总体统计用于描述已知数据集。',
      'stats.trimFractionLabel': '修剪比例（可选）',
      'stats.trimFractionPlaceholder': '留空或 0 表示关闭',
      'stats.trimFractionHint': '仅用于描述统计；每端按 floor(n × 修剪比例) 删除数据后计算均值。',
      ...
      'stats.metricMean': '均值',
      'stats.metricTrimmedMean': '修剪均值',
      'stats.metricMeanCiLower': '均值 95% CI 下限',
      'stats.metricMeanCiUpper': '均值 95% CI 上限',
      'stats.metricMeanCiMargin': '均值 95% CI 半宽',
      'stats.metricMeanSampleSeForCi': 'CI 样本标准误差',
      'stats.metricWeightedSeKnownSigma': '已知 σ 加权 CI 标准误差',
      'stats.metricMeanCiDof': 'CI 自由度',
      'stats.metricMeanCiCritical': 'CI 临界值',
      'stats.metricMeanCiMethod': 'CI 方法',
      'stats.metricStd': '标准差',
      'stats.metricMin': '最小值',
      'stats.metricMax': '最大值',
      'stats.metricEffectiveN': '有效点数 n_eff',
      'stats.metricWeightedChiSquare': '加权均值 χ²',
      'stats.metricWeightedConsistencyDof': '加权一致性自由度',
      'stats.metricWeightedReducedChiSquare': '加权约化 χ²',
      'stats.metricBirgeRatio': 'Birge 比率',
      'stats.metricZeroSigmaAnchor': '零 σ 锚定',
      ...
      // Root solving page
      'roots.eyebrow': '求根 / 方程组',
      'roots.title': '高精度求解单个方程或方程组的根。',
      'roots.subtitle': '支持单方程（标量）与方程组（联立）求解，可选不确定度传播，基于 mpmath 高精度。',
      'roots.chip1': '标量 / 联立',
      'roots.chip2': 'mpmath 精度',
      'roots.chip3': '不确定度传播',
      'roots.equationsLabel': '方程',
      'roots.equationsHint': '每行一个方程，写成 <code>= 0</code> 的左端表达式（如 <code>x**2 - 2</code>）。',
      'roots.equationsExample': '方程组示例：<code>x + y - 3</code> 与 <code>x - y - 1</code>（每行一个）。',
      'roots.equationsPlaceholder': 'x**2 - 2',
      'roots.unknownsLabel': '未知量',
      'roots.unknownsHint': '每行一个未知量与初值，格式 <code>名称 = 初值</code>。',
      'roots.unknownsExample': '示例：<code>x = 1</code>；方程组则每个未知量一行，如 <code>x = 0</code> 与 <code>y = 0</code>。',
      'roots.unknownsPlaceholder': 'x = 1',
      'roots.modeLabel': '求解模式',
      'roots.modeHint': '选择单方程或方程组求解。',
      'roots.modeScalar': '单方程（标量）',
      'roots.modeSystem': '方程组（联立）',
      'roots.uncertaintyLabel': '不确定度方法',
      'roots.uncertaintyOff': '关闭',
      'roots.uncertaintyTaylor': '泰勒展开',
      'roots.uncertaintyMonteCarlo': '蒙特卡洛',
      'roots.uncertaintyHint': '仅当方程含带不确定度的常量时生效。',
      'roots.displayDigitsLabel': '结果显示位数',
      'roots.btnSubmit': '求解方程',
      'roots.submitHint': '从给定初值出发数值求根。',
      'roots.resultsTitlePrefix': '求根结果 · 模式：',
      'roots.backend': '后端',
      'roots.rootsLabel': '根',
      'roots.colName': '未知量',
      'roots.colValue': '值',
      'roots.colUncertainty': '不确定度',
      'roots.latexLabel': 'LaTeX',
      ...
      'errors.missing_equations': '请至少输入一个方程。',
      'errors.missing_unknowns': '请至少输入一个未知量。',
      ...
      'errors.units_unsupported_on_web': '当前模式下不支持启用单位运算（Web 端为只读单位标注）。',
    ...
    en: {
      ...
      'fit.mcmcRefineCheckbox': 'MCMC refinement (polynomial / inverse-power)',
      ...
      'fit.modeComparison': 'Selected-fit comparison',
      'fit.modeSelfConsistent': 'Self-consistent / implicit model',
      ...
      'fit.implicitEquationLabel': 'Implicit equation',
      'fit.implicitEquationPlaceholder': 'e.g., y - A*exp(-B/x*y)',
      'fit.implicitVariableLabel': 'Implicit variable',
      'fit.implicitVariablePlaceholder': 'e.g., y',
      'fit.implicitOutputLabel': 'Output expression',
      'fit.implicitOutputPlaceholder': 'e.g., y',
      'fit.implicitParamsLabel': 'Parameter config (JSON, optional)',
      'fit.implicitInitialLabel': 'Initial value',
      'fit.implicitToleranceLabel': 'Convergence tolerance',
      'fit.implicitMaxIterLabel': 'Max iterations',
      ...
      'fit.comparisonCandidatesLabel': 'Selected fits (JSON)',
      'fit.comparisonCandidatesHint': 'Only the fits explicitly listed here are compared; JSON list order is preserved.',
      'fit.comparisonResultsLabel': 'Selected-fit comparison',
      'fit.comparisonResultsHint': 'Shows status and metrics for each fit in input order.',
      'fit.metricChiSquarePValue': 'χ² p-value',
      'fit.metricMaxStandardizedResidual': 'Max standardized residual',
      'fit.correlationsLabel': 'Parameter Correlations',
      'fit.correlationsHint': 'Covariance-derived correlation coefficients.',
      'fit.correlationsParamI': 'Parameter i',
      'fit.correlationsParamJ': 'Parameter j',
      'fit.correlationsValue': 'Correlation',
      'fit.residualsLabel': 'Standardized Residuals',
      'fit.residualsHint': 'Sigma, weight, or RMSE-normalized residual diagnostics.',
      'fit.residualsRow': 'Row',
      'fit.residualsValue': 'Value',
      'fit.residualsType': 'Type',
      'fit.compareOrder': 'Order',
      'fit.compareId': 'ID',
      'fit.compareModel': 'Model',
      'fit.compareStatus': 'Status',
      'fit.compareParams': 'Parameters',
      'fit.compareWarnings': 'Warnings',
      'fit.compareError': 'Error',
      'fit.exportComparison': 'Export comparison CSV',
      ...
      'stats.modeDescriptive': 'Descriptive statistics',
      'stats.modeWeighted': 'σ weighted mean',
      'stats.metricVariance': 'Variance',
      'stats.metricMedian': 'Median',
      'stats.metricQ1': 'Q1',
      'stats.metricQ3': 'Q3',
      'stats.metricIqr': 'IQR',
      'stats.metricMad': 'MAD',
      'stats.metricSkewness': 'Skewness',
      'stats.metricExcessKurtosis': 'Excess kurtosis',
      ...
      'stats.trimFractionLabel': 'Trim fraction (optional)',
      'stats.trimFractionPlaceholder': 'Blank or 0 disables trimming',
      'stats.trimFractionHint': 'Descriptive statistics only. Each tail removes floor(n × trim fraction) values before averaging.',
      ...
      'stats.metricTrimmedMean': 'Trimmed mean',
      'stats.metricMeanCiLower': 'Mean 95% CI lower',
      'stats.metricMeanCiUpper': 'Mean 95% CI upper',
      'stats.metricMeanCiMargin': 'Mean 95% CI margin',
      'stats.metricMeanSampleSeForCi': 'CI sample SE',
      'stats.metricWeightedSeKnownSigma': 'Known-sigma weighted CI SE',
      'stats.metricMeanCiDof': 'CI dof',
      'stats.metricMeanCiCritical': 'CI critical value',
      'stats.metricMeanCiMethod': 'CI method',
      ...
      'stats.metricWeightedChiSquare': 'Weighted mean chi-square',
      'stats.metricWeightedConsistencyDof': 'Weighted consistency dof',
      'stats.metricWeightedReducedChiSquare': 'Weighted reduced chi-square',
      'stats.metricBirgeRatio': 'Birge ratio',
      'stats.metricZeroSigmaAnchor': 'Zero-sigma anchor',
      ...
      // Root solving page
      'roots.eyebrow': 'Root Solving / Systems',
      'roots.title': 'Solve the roots of a single equation or a system of equations at high precision.',
      'roots.subtitle': 'Supports single-equation (scalar) and system solves with optional uncertainty propagation, backed by mpmath precision.',
      'roots.chip1': 'Scalar / System',
      'roots.chip2': 'mpmath precision',
      'roots.chip3': 'Uncertainty propagation',
      'roots.equationsLabel': 'Equations',
      'roots.equationsHint': 'One equation per line, as the left-hand side of <code>= 0</code> (e.g. <code>x**2 - 2</code>).',
      'roots.equationsExample': 'System example: <code>x + y - 3</code> and <code>x - y - 1</code> (one per line).',
      'roots.equationsPlaceholder': 'x**2 - 2',
      'roots.unknownsLabel': 'Unknowns',
      'roots.unknownsHint': 'One unknown and initial guess per line, as <code>name = guess</code>.',
      'roots.unknownsExample': 'Example: <code>x = 1</code>; for a system, one unknown per line, e.g. <code>x = 0</code> and <code>y = 0</code>.',
      'roots.unknownsPlaceholder': 'x = 1',
      'roots.modeLabel': 'Solve mode',
      'roots.modeHint': 'Choose single-equation or system solving.',
      'roots.modeScalar': 'Single equation (scalar)',
      'roots.modeSystem': 'System (simultaneous)',
      'roots.uncertaintyLabel': 'Uncertainty method',
      'roots.uncertaintyOff': 'Off',
      'roots.uncertaintyTaylor': 'Taylor expansion',
      'roots.uncertaintyMonteCarlo': 'Monte Carlo',
      'roots.uncertaintyHint': 'Only applies when equations contain constants with uncertainty.',
      'roots.displayDigitsLabel': 'Result display digits',
      'roots.btnSubmit': 'Solve equations',
      'roots.submitHint': 'Numerically find roots starting from the given initial guesses.',
      'roots.resultsTitlePrefix': 'Root Solving Results · Mode:',
      'roots.backend': 'Backend',
      'roots.rootsLabel': 'Roots',
      'roots.colName': 'Unknown',
      'roots.colValue': 'Value',
      'roots.colUncertainty': 'Uncertainty',
      'roots.latexLabel': 'LaTeX',
      ...
      'errors.missing_equations': 'Please enter at least one equation.',
      'errors.missing_unknowns': 'Please enter at least one unknown.',
      ...
      'errors.units_unsupported_on_web': 'Active unit evaluation is not supported in this mode (web units are display-only).',

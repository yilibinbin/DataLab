...
def normalize_statistics_bootstrap_options(inputs: Mapping[str, Any]) -> StatisticsBootstrapOptions:
    ...
    if method != BOOTSTRAP_METHOD:
        raise ValueError("Bootstrap method is fixed to percentile in the first release.")
    ...
    if target not in BOOTSTRAP_TARGETS:
        raise ValueError(f"Unsupported bootstrap target statistic: {target}.")
    ...
    if mp.mpf(confidence_level) != mp.mpf(BOOTSTRAP_CONFIDENCE_LEVEL):
        raise ValueError("Bootstrap confidence_level is fixed to 0.95 in the first release.")
    ...
    if resample_count < BOOTSTRAP_MIN_RESAMPLE_COUNT:
        raise ValueError(f"resample_count must be at least {BOOTSTRAP_MIN_RESAMPLE_COUNT}.")
    if resample_count > BOOTSTRAP_MAX_RESAMPLE_COUNT:
        raise ValueError(f"resample_count must be at most {BOOTSTRAP_MAX_RESAMPLE_COUNT}.")
    ...
    if sample_mode not in {"sample", "population"}:
        raise ValueError("sample_mode must be 'sample' or 'population'.")
...
def run_statistics_bootstrap(
    *,
    ...
    if any(not mp.isfinite(value) for value in values_mp):
        raise ValueError("Bootstrap statistics requires finite numeric values.")
    if options.target_statistic in {"std", "variance"} and options.sample_mode == "sample" and len(values_mp) < 2:
        raise ValueError("Sample std/variance bootstrap requires at least two values.")

    row_ids = tuple(source_row_ids or tuple(str(index + 1) for index in range(len(values_mp))))
    if len(row_ids) != len(values_mp):
        raise ValueError("source_row_ids must have the same length as values.")
    ...
    if not mp.isfinite(original):
        raise ValueError("Original bootstrap statistic is not finite.")
    ...
    if not finite_values:
        raise ValueError("No finite bootstrap replicates were produced.")
...
def validate_statistics_bootstrap_payload(payload: Mapping[str, Any]) -> None:
    ...
    if payload.get("schema") != BOOTSTRAP_PAYLOAD_SCHEMA:
        raise ValueError("Invalid bootstrap payload schema.")
    if payload.get("workflow_mode") != BOOTSTRAP_WORKFLOW_MODE:
        raise ValueError("Invalid bootstrap workflow mode.")
    if payload.get("target_statistic") not in BOOTSTRAP_TARGETS:
        raise ValueError("Invalid bootstrap target statistic.")
    ...
    if not isinstance(columns, Sequence) or isinstance(columns, (str, bytes, bytearray, memoryview)) or not columns:
        raise TypeError("columns must be a non-empty sequence.")
    for index, column in enumerate(columns):
        if not isinstance(column, Mapping):
            raise TypeError("bootstrap columns must be mappings.")
...
def statistics_bootstrap_analysis_rows_from_column(
    ...
    if not isinstance(distribution, Mapping):
        raise TypeError("bootstrap column distribution must be a mapping.")
    percentiles = distribution["percentiles"]
    if not isinstance(percentiles, Mapping):
        raise TypeError("bootstrap distribution percentiles must be a mapping.")
...
def _evaluate_target(
    ...
    *,
    ...
    if target_statistic == "trimmed_mean":
        ...
        if trimmed is None:
            raise ValueError("trimmed_mean requires trim_fraction.")
    ...
    raise ValueError(f"Unsupported bootstrap target statistic: {target_statistic}.")
...
def _distribution_summary_to_payload(summary: Mapping[str, Any], precision_digits: int) -> dict[str, object]:
    ...
    if not isinstance(histogram, Mapping):
        raise ValueError("distribution histogram must be a mapping.")
    percentiles = summary.get("percentiles")
    if not isinstance(percentiles, Mapping):
        raise ValueError("distribution percentiles must be a mapping.")
    return {
...
def _validate_bootstrap_column(column: Mapping[str, Any], *, resample_count: int, index: int) -> None:
    ...
    if not isinstance(value_column, str):
        raise TypeError("bootstrap column value_column must be a string.")
    ...
    if column_index is not None and (isinstance(column_index, bool) or not isinstance(column_index, int) or column_index < 1):
        raise TypeError("bootstrap column column_index must be a positive integer.")
    row_count = column.get("row_count")
    if isinstance(row_count, bool) or not isinstance(row_count, int) or row_count < 1:
        raise TypeError("bootstrap column row_count must be a positive integer.")
    ...
    if (
        ...
        raise ValueError("source_row_ids must match row_count.")
    ...
    for duplicate_key in ("mean", "std", "ci_lower", "ci_upper", "finite_sample_count"):
        if duplicate_key in column:
            raise ValueError(f"bootstrap column duplicates distribution field: {duplicate_key}.")
    diagnostics = column.get("diagnostics")
    if not _is_text_sequence(diagnostics):
        raise TypeError("bootstrap column diagnostics must be a sequence of strings.")
...
def _validate_distribution_summary_payload(summary: Mapping[str, Any], *, resample_count: int) -> None:
    ...
    if summary.get("schema") != MONTE_CARLO_DISTRIBUTION_SUMMARY_SCHEMA:
        raise ValueError("Invalid distribution summary schema.")
    if summary.get("schema_version") != MONTE_CARLO_DISTRIBUTION_SUMMARY_SCHEMA_VERSION:
        raise ValueError("Invalid distribution summary schema version.")
    ...
    if requested_count != resample_count:
        raise ValueError("distribution requested_sample_count must match resample_count.")
    if evaluated_count != resample_count:
        raise ValueError("distribution evaluated_sample_count must match resample_count.")
    if accepted_count != finite_count:
        raise ValueError("distribution accepted_sample_count must equal finite_sample_count.")
    if accepted_count + rejected_count != resample_count:
        raise ValueError("distribution accepted/rejected counts must match resample_count.")
    if accepted_count <= 0:
        raise ValueError("distribution must contain at least one accepted sample.")
    _require_numeric_string(summary.get("mean"), "distribution.mean")
    _require_numeric_string(summary.get("std"), "distribution.std")
    histogram = summary.get("histogram")
    if not isinstance(histogram, Mapping):
        raise TypeError("distribution histogram must be a mapping.")
    ...
    if not isinstance(edges, Sequence) or isinstance(edges, (str, bytes, bytearray, memoryview)):
        raise TypeError("distribution histogram bin_edges must be a sequence.")
    if not isinstance(counts, Sequence) or isinstance(counts, (str, bytes, bytearray, memoryview)):
        raise TypeError("distribution histogram counts must be a sequence.")
    for edge in edges:
        _require_numeric_string(edge, "distribution.histogram.bin_edges[]")
    for count in counts:
        if isinstance(count, bool) or not isinstance(count, int) or count < 0:
            raise TypeError("distribution histogram counts must be non-negative integers.")
    if counts:
        if len(edges) != len(counts) + 1:
            raise ValueError("distribution histogram must have one more edge than count.")
    elif edges:
        raise ValueError("distribution histogram edges require counts.")
    if sum(int(count) for count in counts) != finite_count:
        raise ValueError("distribution histogram counts must sum to finite_sample_count.")
    percentiles = summary.get("percentiles")
    if not isinstance(percentiles, Mapping):
        raise TypeError("distribution percentiles must be a mapping.")
...
def _bootstrap_option_inputs(inputs: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for key in _BOOTSTRAP_OPTION_KEYS:
        ...
        if has_nested and has_top_level and nested[key] != inputs[key]:
            raise ValueError(f"Conflicting bootstrap option: {key}.")
        if has_nested:
...
def _nonnegative_int(value: Any, field_name: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or value < 0:
        raise TypeError(f"distribution {field_name} must be a non-negative integer.")
    return int(value)
...
def _validate_source_row_id(value: Any, *, field_name: str) -> None:
    if isinstance(value, float):
        raise TypeError(f"JSON floats are not allowed at {field_name}.")

...
def run_statistics_matrix(*, precision_digits: int, inputs: Mapping[str, Any]) -> dict[str, Any]:
    ...
    with precision_guard(precision_digits) as precision_used:
        ...
        if len(columns) < 2:
            raise ValueError("covariance/correlation requires at least two value columns.")
        if len(set(columns)) != len(columns):
            raise ValueError("covariance/correlation value columns must be unique.")
    ...
    if not isinstance(normalized, Mapping):
        raise TypeError("statistics matrix payload must normalize to a mapping.")
    return dict(normalized)


def validate_statistics_matrix_payload(payload: Mapping[str, Any]) -> None:
    if not isinstance(payload, Mapping):
        raise TypeError("statistics matrix payload must be a mapping.")
    ...
    if payload.get("schema") != MATRIX_PAYLOAD_SCHEMA:
        raise ValueError("statistics matrix payload schema is unsupported.")
    if payload.get("workflow_mode") != MATRIX_WORKFLOW_MODE or payload.get("mode") != MATRIX_WORKFLOW_MODE:
        raise ValueError("statistics matrix workflow mode is unsupported.")
    columns = _required_text_list(payload.get("columns"), "columns")
    if len(columns) < 2 or len(set(columns)) != len(columns):
        raise ValueError("statistics matrix columns must contain at least two unique names.")
    size = len(columns)
    ...
    if metadata.get("source") != "statistics_covariance_correlation":
        raise ValueError("correlation metadata source is unsupported.")
    expected_alignment = "pairwise" if missing_policy == "pairwise" else "listwise"
    if metadata.get("row_alignment") != expected_alignment:
        raise ValueError("correlation metadata row_alignment does not match missing_policy.")
    if metadata.get("weighted") is not False:
        raise ValueError("first-release statistics matrix payloads must be unweighted.")
    budget_eligible = metadata.get("budget_eligible")
    if not isinstance(budget_eligible, bool):
        raise TypeError("correlation_metadata.budget_eligible must be boolean.")
    ...
    if budget_eligible and (missing_policy != "listwise" or not finite_correlation):
        raise ValueError("budget_eligible requires listwise finite correlation cells.")
...
def build_statistics_matrix_result_snapshot(
    ...
    *,
    ...
    if not isinstance(normalized_payload, Mapping):
        raise TypeError("statistics matrix snapshot payload must normalize to a mapping.")
    ...
    if not isinstance(normalized_snapshot, Mapping):
        raise TypeError("statistics matrix snapshot must normalize to a mapping.")
...
def validate_statistics_matrix_snapshot(snapshot: Mapping[str, Any]) -> None:
    if not isinstance(snapshot, Mapping):
        raise TypeError("statistics matrix snapshot must be a mapping.")
    ...
    if snapshot.get("schema") != MATRIX_RESULT_SNAPSHOT_SCHEMA:
        raise ValueError("statistics matrix snapshot schema is unsupported.")
    if snapshot.get("schema_version") != MATRIX_RESULT_SNAPSHOT_SCHEMA_VERSION:
        raise ValueError("statistics matrix snapshot schema_version is unsupported.")
    if snapshot.get("family") != "statistics" or snapshot.get("mode") != MATRIX_WORKFLOW_MODE:
        raise ValueError("statistics matrix snapshot family/mode is unsupported.")
    ...
    if _required_text_list(source.get("value_columns"), "source.value_columns") != payload_columns:
        raise ValueError("statistics matrix snapshot source columns do not match payload.")
    if source.get("column_count") != len(payload_columns):
        raise ValueError("statistics matrix snapshot source column_count does not match payload.")
    for key in ("missing_policy", "denominator", "row_count", "input_row_count"):
        if source.get(key) != payload.get(key):
            raise ValueError(f"statistics matrix snapshot source {key} does not match payload.")
    if list(_required_text_list(source.get("source_row_ids"), "source.source_row_ids")) != list(
        ...
        raise ValueError("statistics matrix snapshot source_row_ids do not match payload.")
    ...
    if matrix_kinds != {"covariance", "correlation"}:
        raise ValueError("statistics matrix snapshot must include covariance and correlation matrices.")
    if list(matrices) != _snapshot_matrices(payload):
        raise ValueError("statistics matrix snapshot matrices do not match payload.")
    ...
    if dict(metadata) != dict(_required_mapping(payload.get("correlation_metadata"), "payload.correlation_metadata")):
        raise ValueError("statistics matrix snapshot correlation metadata does not match payload.")
    ...
    if compatibility.get("result_cache_kind") != MATRIX_RESULT_CACHE_KIND:
        raise ValueError("statistics matrix snapshot compatibility kind is unsupported.")
...
def _listwise_payload(
    *,
    ...
    if not included:
        raise ValueError("covariance/correlation listwise policy left no complete rows.")
    ...
    for row_index, row in enumerate(rows):
        ...
        for column, column_index in zip(columns, column_indexes):
            if column_index >= len(row):
                raise ValueError(f"Row {row_index + 1} is missing column {column}.")
            values.append(_nullable_mpf(row[column_index], field_name=f"rows[{row_index}][{column}]"))
        collected.append(tuple(values))
    if not collected:
        raise ValueError("covariance/correlation rows must not be empty.")
...
def _nullable_mpf(value: Any, *, field_name: str) -> mp.mpf | None:
    ...
    try:
        ...
        raise ValueError(f"{field_name} is not a valid number or supported missing marker: {value!r}.") from exc
...
def _selected_indexes(headers: tuple[str, ...], columns: tuple[str, ...]) -> tuple[int, ...]:
    ...
    for column in columns:
        ...
        if count > 1:
            raise ValueError(f"Column is ambiguous: {column}.")
...
def _row_sequence(value: Any, *, field_name: str) -> tuple[tuple[Any, ...], ...]:
    ...
    for index, row in enumerate(value):
        if not isinstance(row, Sequence) or isinstance(row, (str, bytes, bytearray, memoryview)):
            raise TypeError(f"{field_name}[{index}] must be a row sequence.")
...
def _source_row_ids(value: Any, *, count: int) -> tuple[str, ...]:
    ...
    if len(ids) != count:
        raise ValueError("source_row_ids length must match row count.")
    return ids
...
def _bool_option(value: Any, *, default: bool) -> bool:
    ...
    if isinstance(value, str):
        ...
        if lowered in {"0", "false", "no", "off"}:
            return False
    raise TypeError("boolean option must be a boolean or boolean string.")
    ...
    for row_index in range(size):
        for column_index in range(size):
            ...
            if counts[row_index][column_index] is None or denominators[row_index][column_index] is None:
                raise ValueError(f"{name} numeric cells require count and denominator.")
    ...
    for row_index, row in enumerate(matrix):
        ...
        for column_index, cell in enumerate(row):
            ...
            if not isinstance(cell, str):
                raise TypeError(f"{name}[{row_index}][{column_index}] must be a string or null.")
...
def _square_matrix(value: Any, *, size: int, name: str) -> list[list[Any]]:
    ...
    if len(value) != size:
        raise ValueError(f"{name} row count must match columns.")
    result: list[list[Any]] = []
    for index, row in enumerate(value):
        if not isinstance(row, Sequence) or isinstance(row, (str, bytes, bytearray, memoryview)):
            raise TypeError(f"{name}[{index}] must be a row sequence.")
        if len(row) != size:
            raise ValueError(f"{name}[{index}] column count must match columns.")
        ...
        for j in range(size):
            ...
            if left is None or right is None:
                if left is not right:
                    raise ValueError(f"{name} null cells must be symmetric.")
                continue
    ...
    for row_index, row in enumerate(values):
        for column_index, cell in enumerate(row):
            ...
            if row_index == column_index and not mp.almosteq(numeric, 1, rel_eps=mp.mpf("1e-30"), abs_eps=mp.mpf("1e-30")):
                raise ValueError("finite correlation diagonal cells must be 1.")
    ...
    for index, item in enumerate(value):
        ...
        for key in ("code", "message"):
            if not isinstance(diagnostic.get(key), str) or not diagnostic.get(key):
                raise ValueError(f"diagnostics[{index}].{key} must be a non-empty string.")
